...
nj=5                # The number of parallel jobs.
inference_nj=5      # The number of parallel jobs in decoding.
...
Options:
    ...
    --num_utt # selects <num-utt> utterances per speaker, if available or ALL
    --num_spk # selects <num-spk> speakers, if available or ALL
    # Speed perturbation related
...
if "${skip_train}"; then
    skip_stages+="1 2 4 5 6 7 8 9 10 11 " #xiaoxiao add 1
elif ! "${use_lm}"; then
    skip_stages+="6 7 8 "
fi
if ! "${use_ngram}"; then
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ] && ! [[ " ${skip_stages} " =~ [[:space:]]1[[:space:]] ]]; then
    ...
    if [ "${num_spk}" != "ALL" ]; then
      echo "selected ${num_spk} speakers"
      #spk_sid
      cat data/${train_set}/spk2utt | awk '{ print $1; }' | utils/shuffle_list.pl  --srand $seed >data/${train_set}/speakers
      #spk
      cat data/${train_set}/spk2utt | awk -F'-' '{print $1}' | sort -n | uniq | utils/shuffle_list.pl  --srand $seed >data/${train_set}/speakers_pure
      head -n ${num_spk} data/${train_set}/speakers_pure >data/${train_set}/selected_speakers_pure
      ## xx added 2021/02/13
      python utils/generate_selected_speakers.py data/${train_set}/selected_speakers_pure data/${train_set}/speakers data/${train_set}/selected_speakers
      utils/subset_data_dir.sh --spk-list data/${train_set}/selected_speakers data/${train_set} data/${train_set}_${num_spk}
      train_set=${train_set}_${num_spk}
    elif [ "${num_spk}" == "ALL" ]; then
      echo "selected all speakers"
	  else
	    log "${num_spk} is not supported yet."
	    exit 1
	  fi

    if [ "${num_utt}" != "ALL" ]; then
      echo "selected ${num_utt} utterances for each selected speaker"
      utils/subset_data_dir.sh --per-spk data/${train_set} ${num_utt} data/${train_set}_${num_utt}_per_spk
      train_set=${train_set}_${num_utt}_per_spk
    elif [ "${num_utt}" == "ALL" ]; then
      echo "selected all utterances for each selected speaker"
    else
      echo "num_utt is ${num_utt}, not supported yet"
	  fi
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    ...
    if [ "${feats_type}" = raw ]; then
	    ...
	    IFS='/' read -ra fields <<< "$dset"
	    num_fields=${#fields[@]}
	    if [ "$num_fields" -gt 3 ]; then
		    filename=$(basename "$dset")
		    echo utils/copy_data_dir.sh --validate_opts --non-print "${dset}" "${data_feats}${_suf}/${filename}"
		    utils/copy_data_dir.sh --validate_opts --non-print "${dset}" "${data_feats}${_suf}/${filename}"
		    rm -f ${data_feats}${_suf}/${filename}/{segments,wav.scp,reco2file_and_channel,reco2dur}
	    else
		    echo utils/copy_data_dir.sh --validate_opts --non-print data/"${dset}" "${data_feats}${_suf}/${dset}"
	    ...
	    if [ "$num_fields" -gt 3 ]; then
                    filename=$(basename "$dset")
		    dir_temp=$(dirname "$dset")
		    dir=$(dirname "${dir_temp}")/
		    num_fields_wav=$(awk '{print NF; exit}' "${dset}/wav.scp")
		    if [ "${num_fields_wav}" -eq 2 ]; then
			    line=$(head -n 1 ${dset}/wav.scp) 
			    echo $line
			    ...
			    if [ "$flag" -eq 1 ]; then
				    awk -v p="$dir" -i inplace '{print $1, p$2}' ${dset}/wav.scp
			    fi
		    fi
		    scripts/audio/format_wav_scp.sh --nj "${nj}" --cmd "${train_cmd}" \
			    --audio-format "${audio_format}" --fs "${fs}" ${_opts} \
			    "${dset}/wav.scp" "${data_feats}${_suf}/${filename}"
		   

		    echo "${feats_type}" > "${data_feats}${_suf}/${filename}/feats_type"
            else
		    scripts/audio/format_wav_scp.sh --nj "${nj}" --cmd "${train_cmd}" \
			    --audio-format "${audio_format}" --fs "${fs}" ${_opts} \
...
if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ] && ! [[ " ${skip_stages} " =~ [[:space:]]12[[:space:]] ]]; then
    ...
    # TODO catch case asr_exp already exists
    ...
    echo "Created ${asr_exp}/${inference_tag}"
